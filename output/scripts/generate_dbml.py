"""Generated ERVE helper script for dbml export."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ERVE dbml export.")
    ...
    result = getattr(engine, "generate_dbml_export", None)
    if result is None:
        if "dbml" == "full":
            payload = engine.generate_full_export()
        else:
            payload = getattr(engine, "generate_dbml")()
    else:
        payload = result()
