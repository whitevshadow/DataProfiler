"""Generated ERVE helper script for drawio export."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ERVE drawio export.")
    ...
    result = getattr(engine, "generate_drawio_export", None)
    if result is None:
        if "drawio" == "full":
            payload = engine.generate_full_export()
        else:
            payload = getattr(engine, "generate_drawio")()
    else:
        payload = result()
