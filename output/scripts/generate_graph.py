"""Generated ERVE helper script for graph export."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ERVE graph export.")
    ...
    result = getattr(engine, "generate_graph_export", None)
    if result is None:
        if "graph" == "full":
            payload = engine.generate_full_export()
        else:
            payload = getattr(engine, "generate_graph")()
    else:
        payload = result()
