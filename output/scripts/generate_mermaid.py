"""Generated ERVE helper script for mermaid export."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ERVE mermaid export.")
    ...
    result = getattr(engine, "generate_mermaid_export", None)
    if result is None:
        if "mermaid" == "full":
            payload = engine.generate_full_export()
        else:
            payload = getattr(engine, "generate_mermaid")()
    else:
        payload = result()
