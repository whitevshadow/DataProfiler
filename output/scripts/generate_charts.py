"""Generated ERVE helper script for charts export."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ERVE charts export.")
    ...
    result = getattr(engine, "generate_charts_export", None)
    if result is None:
        if "charts" == "full":
            payload = engine.generate_full_export()
        else:
            payload = getattr(engine, "generate_charts")()
    else:
        payload = result()
