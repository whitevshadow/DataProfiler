"""Generated ERVE helper script for full export."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ERVE full export.")
    ...
    result = getattr(engine, "generate_full_export", None)
    if result is None:
        if "full" == "full":
            payload = engine.generate_full_export()
        else:
            payload = getattr(engine, "generate_full")()
    else:
        payload = result()
